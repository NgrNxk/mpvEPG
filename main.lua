...
Copyright © 2026 Adam Duck; MIT Licensed
See https://github.com/NgrNxk/mpvEPG for details.
--]] --
---========================== SLAXML PART START ============================---
-------------------------------------------------------------------------------
...
function SLAXML:parse(xml, options)
    ...
    local find, sub, gsub, char, push, pop, concat = string.find, string.sub, string.gsub, string.char, table.insert,
...
function SLAXML:dom(xml, opts)
    ...
    local builder = SLAXML:parser({
        startElement = function(name, nsURI, nsPrefix)
            ...
            if current == doc then
                if doc.root then
                    error(("Encountered element '%s' when the document already has a root '%s' element"):format(name,
                        doc.root.name))
                end
        ...
        closeElement = function(name)
            if current.name ~= name or current.type ~= "element" then
                error(("Received a close element notification for '%s' but was inside a '%s' %s"):format(name,
        ...
        text = function(value, cdata)
            ...
            if current.type == "document" and not value:find("^%s+$") then
                error(("Document has non-whitespace text at root: '%s'"):format(value:gsub("[\r\n\t]", {
                    ["\r"] = "\\r",
                    ["\n"] = "\\n",
                    ["\t"] = "\\t"
                })))
            ...
            if n.attr["channel"] == channel and (progDate == date or progDate == yesterday) then
                ...
                if progStart <= dateLong and progStop >= dateLong then
                    ...
                    if prog_subtitle ~= "" then
                        now.subtitle = string.format("{\\bord2\\fs%s\\b1\\i1\\1c&H%s}⦗%s-%s⦘{\\b0}- %s\\N\\N",
                            opts.subtitleSize, opts.subtitleColor, start, stop, prog_subtitle)
                ...
                elseif progStart > dateLong then
                    ...
                    if display_mode >= 1 and (opts.max_upcoming == 0 or #program < opts.max_upcoming) then
                        local entry = string.format("{\\b1\\be\\fs%s\\1c&H%s}⦗%s – %s⦘{\\b0\\fs%s} %s\\N",
                            opts.upcomingTimeSize, opts.upcomingColor, start, stop, opts.upcomingTitleSize, prog_title)
                        -- add description if display_mode is 2 and description is available
                        if display_mode == 2 and prog_desc ~= "" then
                            entry = entry ..
                                        string.format("{\\bord2\\fs%s\\1c&H%s}%s\\N", opts.upcomingDescSize,
                                    opts.upcomingDescColor, prog_desc)
                        end
